...
namespace BookSortingEx
{
    class Program
    {
        ...
        static public void SelectionSort(Book[] library)
        {
            for (int i = 0; i < library.Length - 1; i++)
            {
                ...
                {
                    Book book1 = library[j];
                    Book book2 = library[smallest];
                    IComparable<Book> BookOrder = (IComparable<Book>)book1;
                    if (BookOrder.CompareTo(book2) == -1) smallest = j;
                }
